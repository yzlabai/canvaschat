...
// Email Magic Link Auth via Credentials Provider
providers.push(
  CredentialsProvider({
    id: "magic-link",
    name: "Magic Link",
    ...
    async authorize(credentials) {
      ...
      try {
        ...
        const { verifyAndConsumeToken } = await import("@/services/verification");
        
        // Verify the token
        const verifiedEmail = await verifyAndConsumeToken(credentials.token as string);
        
        if (!verifiedEmail || verifiedEmail !== credentials.email) {
          return null;
        }
...
export const providerMap = providers
  ...
  .filter((provider) => provider.id !== "google-one-tap" && provider.id !== "magic-link");
...
export const authOptions: NextAuthConfig = {
  ...
  callbacks: {
    async signIn({}) {
      //TODO if want to add additional sign-in logic
      return true;
    },
