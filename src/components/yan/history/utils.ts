import type { ConversationWithStats } from "@/lib/chat-store/types"

type TimeGroup = {
  name: string
  chats: ConversationWithStats[]
}
...
export function groupChatsByDate(
  ...
  const todayChats: ConversationWithStats[] = []
  const last7DaysChats: ConversationWithStats[] = []
  const last30DaysChats: ConversationWithStats[] = []
  const thisYearChats: ConversationWithStats[] = []
  const olderChats: Record<number, ConversationWithStats[]> = {}
