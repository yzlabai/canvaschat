...
export default memo(
  ({
    ...
    const videoPollingRef = useRef<
      Map<
        string,
        { cancelled: boolean; timeout?: ReturnType<typeof setTimeout> }
      >
    >(new Map());
    ...
    const isGenerationBusy = isVideoBusy || hasPendingGeneration;
      ...
      (mediaId: string, pollUrl?: string) => {
        ...
        const poll = async () => {
          ...
          try {
            ...
            if (!response.ok) {
              throw new Error(
                `Failed to poll video status: ${response.status}`
              );
            }
      ...
      async (mediaId: string, mediaType?: string | null) => {
        ...
        try {
          if (mediaType === "video") {
            ...
            const response = await fetch(
              `/api/yan/ideas/gen/video?media_id=${mediaId}`,
              {
                cache: "no-store",
              }
            );
    ...
    const displayTitle = useMemo(() => {
      ...
      return rawTitle.length > 0 ? rawTitle : "Video concept";
    }, [node.title]);
